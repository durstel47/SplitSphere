...
namespace SMI
{
    public partial class SMIGazeController : MonoBehaviour
    {
        ...
        {
#if (UNITY_EDITOR_64 || UNITY_STANDALONE_64 || UNITY_64)
            const string dllName = "iViewHMDAPI_x64.dll";   // 64bit library
#else
            const string dllName = "iViewHMDAPI.dll";       // default: 32bit library
#endif
...
#if EYEMONITOR
            [DllImport(dllName, CallingConvention = CallingConvention.StdCall, EntryPoint = "smi_showEyeImageMonitor")]
            // public static extern smi_EyeImageStruct smi_showEyeImageMonitor(int xl = 0, int yl = 0, int xr = 320, int yr = 0);
            public static extern int smi_showEyeImageMonitor(int xl = 0, int yl = 0, int xr = 0, int yr = 400);

            [DllImport(dllName, CallingConvention = CallingConvention.StdCall, EntryPoint = "smi_hideEyeImageMonitor")]
            public static extern int smi_hideEyeImageMonitor();


            [DllImport(dllName, CallingConvention = CallingConvention.StdCall, EntryPoint = "smi_startEyeImageStreaming")]
            public static unsafe extern int smi_startEyeImageStreaming();

            [DllImport(dllName, CallingConvention = CallingConvention.StdCall, EntryPoint = "smi_stopEyeImageStreaming")]
            public static unsafe extern int smi_stopEyeImageStreaming();
...
#region HelperClasses
            ...
            public class DefaultCalibrationInformations
            {
                ...
                private static Vector2[] defaultNinePointCalibration = 
                {  
                ...
                private static Dictionary<int, string> ErrorId = new Dictionary<int, string>()
                    {
                        ...
                        {513,"SMI_ERROR_MRD_HARDWARE"} //MRDs addition to avoid nasty error
                    };
